...
(function() {
  ...
  /**
   * Loads the rules from our config file
   * @param  {[options]} options [defines where the
   * config file is located, and the encoding type]
   * @return {[JSON]}
   */
  var config = function(options) {
    var path, rules, encoding, yml, filePath;
    var fileName = 'nacl.json';
    ...
    fileName = yml ? 'nacl.yml' : fileName;
    filePath = fileName && path ? (path + '/' + fileName) : fileName;
    rules = helper.getRules(filePath, encoding, yml);
